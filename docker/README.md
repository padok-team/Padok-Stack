...
## About multiple Git repos and their naming

As you may already know (or not), the create-bam-stack git repo is kind of a bootstrapping repo for BAM stack project.
...
Those two repos will be dedicated to two different parts of the new project:

1. **The API**
2. **The backoffice**

And then there is the repo whose README you're currently reading ;-)
So that's 4 different repos...

For clarity purposes, and in order not to take one repo for an other during this procedure,
they will be referred in the next part as follow:
...
We will use variables in this README to make it as general as possible.

Variables will always be noted between a `<` and a `>`.

For example the variable whose name is "toto" will be written `<toto>` in any section, any paragraph or code sample.

Among the variable that we will use, we can identify:
- **<project_name>:** your project's name, written in a raw form (no spaces)
- **<beautiful_project_name>:** also your project name but maybe camel cased so it will be easier to read (no spaces)

**Note:**
- Repos are a special case of variable. To simplify the writing we will use for example **"repo-padok"** to indicate both the **repo** and **its path**.
  This goes for the other repos names.
...
$ cd <project_name>
```
If you have configured your ssh key with your GitHub account, you can clone over ssh instead:
```bash
$ git clone git@github.com:bamlab/create-bam-stack.git <project_name>
```

Now you will use the `repo-bam/setup.sh` script to create **repo-api** and **repo-bo**.

But before you can use it, you may need to customize the script a little:
...
Now if you look inside the current dir you will see an output like the following:
...
If you're using VSCode or any other IDE, you can use its refactoring capabilities.

If you are not, you can also use a command like the following:
...
**Note for Mac users:** These commands may fail. In that case you can replace these patterns using your IDE instead.

You should end up with the following in the current dir:
...
Ok, now that you have BAM-like repos you are ready to dockerize them.
...
**Note:** the `repo-padok/config/default.yaml` file available in this repo should be enough to perform a PoC test as well as build more complex use cases.
...
- Copy it in `repo-api/config/firebase-adminsdk.json`
...
- A Node, called "node-lr", that will run a dev-like config
...
Each node container depends on the database, so you only need to ask compose to launch a node and the database will be launched before automatically

Which means you only have to go to the repo-api dir and perform on of the following commands:

 - `docker-compose up node`: for a production-like environment
 - `docker-compose up node-lr`: for a debug-like environment

For example, if you want to launch the node-lr version

```bash
$ cd repo-api
$ docker-compose up node-lr
```

**Tip:** Just ad `-d` flag if you don't want docker-compose to stay in the foreground

```bash
$ docker-compose up -d node-lr
```

## Test the app

If everything went fine in the previous part, your api is now listening, let's say on port 3000.
